...
import {
  ...
  MapPin, CalendarDays, CalendarRange, LayoutGrid, GraduationCap, ChevronDown
} from 'lucide-react';
...
const ParentTimetablePage = ({ selectedChild: initialChild, children: propChildren }) => {
  // Get children from props or localStorage
  ...
  // Fetch children from localStorage if not passed as props
  useEffect(() => {
      ...
      try {
        ...
        if (userInfo) {
          ...
          if (parsed.children && parsed.children.length > 0) {
            setAllChildren(parsed.children);
          }
        }
      } catch (e) {
  ...
  const fetchTimetable = async () => {
    if (!selectedChild?.classSectionId || !academicSession?.id) {
      console.log('Missing data:', { classSectionId: selectedChild?.classSectionId, academicSessionId: academicSession?.id });
      setLoading(false);
      return;
    }
    ...
    setLoading(true);
    try {
      console.log('Fetching timetable for class section:', selectedChild.classSectionId);
    ...
    return (
      <div className="flex items-center justify-center h-64">
        <div className="text-center">
          ...
          <h3 className="text-lg font-semibold text-white mb-2">No Child Selected</h3>
          <p className="text-slate-400">Please select a child to view their timetable</p>
        </div>
      </div>
    );
  }

  // Show message if class section info is missing for the selected child
  if (!loading && !selectedChild?.classSectionId) {
    return (
      <div className="flex items-center justify-center h-64">
        <div className="text-center bg-slate-800 rounded-xl p-8 border border-slate-700">
          ...
          <p className="text-slate-400">{selectedChild?.fullName}'s class section is not assigned yet.</p>
          <p className="text-slate-500 text-sm mt-2">Please contact school administrator.</p>
        </div>
      </div>
    );
  }
          ...
          {allChildren.length > 0 && (
            <div className="relative">
              <button
                ...
                className="flex items-center gap-2 px-4 py-2 bg-slate-800 rounded-lg border border-slate-700 hover:border-slate-600 transition-all"
              >
              ...
              {showChildSelector && allChildren.length > 1 && (
                <div className="absolute top-full left-0 right-0 mt-1 bg-slate-800 rounded-lg shadow-lg overflow-hidden z-10 border border-slate-700">
                  {allChildren.map(child => (
                    <button
                      ...
                      className={`w-full p-3 text-left hover:bg-slate-700 flex items-center gap-3 ${
                        selectedChild?.id === child.id ? 'bg-slate-700' : ''
                      }`}
                    >
                      <div className="w-8 h-8 rounded-full bg-gradient-to-br from-blue-500 to-cyan-500 flex items-center justify-center text-white text-sm font-bold">
                        {child.firstName?.[0]}
                        ...
                        >
                          <div className="flex items-center justify-between">
                            <div className="flex items-center gap-3">
                              <span className={`text-2xl ${entry.periodType === 'lunch' ? '' : ''}`}>
                                {entry.periodType === 'lunch' ? '🍱' : '☕'}
                              </span>
                              <div>
                  ...
                  return (
                    <div
                      ...
                      className={`bg-slate-800 rounded-xl border ${
                        isToday ? 'border-amber-500' : 'border-slate-700'
                      }`}
                    >
                      <div className={`p-3 border-b border-slate-700 ${isToday ? 'bg-amber-500/20' : ''}`}>
                        <p className={`font-semibold ${isToday ? 'text-amber-400' : 'text-white'}`}>{day}</p>
